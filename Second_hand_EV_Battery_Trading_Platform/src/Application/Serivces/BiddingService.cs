...
public class BiddingService : IBiddingService
{
    ...
    public async Task<PlaceProxyBidResponseDto> PlaceProxyBidAsync(PlaceProxyBidRequestDto request)
    {
        ...
        if (request.MaxBidAmount < listing.StartPrice)
        {
            throw new InvalidOperationException("Max bid amount must be at least the starting price");
        }
        if (request.MaxBidAmount <= 0)
        {
            throw new InvalidOperationException("Max bid amount must be greater than zero");
        }
