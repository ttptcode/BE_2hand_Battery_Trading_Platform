...
const MOTO_BRANDS = [
  "Aprilia", "Avent", "Bazan", "Before All", "Benelli", "BMW", "Brixton", "CR&S",
  "Daelim", "Datbike", "Detech", "Dibao", "DKBike", "Ducati", "Euro Reibel", "GPX",
  "Halim", "Harley Davidson", "Honda", "Husqvarna", "Hyosung", "JVC", "Kawasaki",
  "Keeway", "Kengo", "KTM", "Kumatsu", "Kymco", "Lambretta", "Lihaze", "Malaguti",
  "Moto Guzzi", "MV Agusta", "Nioshima", "Norton", "Osakar", "Peugeot", "Piaggio",
  "RebelUSA", "Regal Raptor", "Royal Enfield", "Sachs", "Sanda", "Suzuki", "SYM",
  "Taya", "Triumph", "Vento", "Victory", "VinFast", "Visitor", "Yadea", "Yamaha",
  "Zontes", "Hãng khác"
];

const VEHICLE_TYPES = ["Tay ga", "Xe số", "Tay côn/Moto"];
const ORIGINS = ["Việt Nam", "Ấn Độ", "Hàn Quốc", "Thái Lan", "Nhật Bản", "Trung Quốc", "Mỹ", "Đức", "Đài Loan", "Nước khác"];
const ENGINE_SIZES = ["Dưới 50 cc", "50 - 100 cc", "100 - 175 cc", "Trên 175 cc", "Không biết rõ"];
const MOTO_YEARS = [
...
const MotoPost = ({ images, itemTypeId, draftData, isEditMode }) => {
  ...
  const [showEngineSizeDropdown, setShowEngineSizeDropdown] = useState(false);
  ...
  const engineSizeDropdownRef = useRef(null);

  const filteredBrands = MOTO_BRANDS.filter((brand) =>
  ...
  const filteredYears = MOTO_YEARS.filter((year) =>
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      if (engineSizeDropdownRef.current && !engineSizeDropdownRef.current.contains(event.target)) {
        setShowEngineSizeDropdown(false);
      }
    };
  ...
  const handleEngineSizeSelect = (size) => {
    setFormData((prev) => ({ ...prev, engineSize: size }));
    setEngineSizeSearchTerm("");
    setShowEngineSizeDropdown(false);
    clearFieldError('engineSize');
  };
  ...
  useEffect(() => {
    if (isEditMode && draftData) {
      const filledData = autoFillFormData(draftData, 'moto');
  ...
  const handlePackageChange = (packageInfo) => {
    ...
    console.log('✅ Package selected in MotoPost:', {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const errors = validateForm(formData, 'moto', uploadedImages);
      ...
      const serialNumber = `MOTO_${formData.brand || 'UNKNOWN'}_${formData.year || 'YEAR'}_${randomStr}_${timestamp}`;
      ...
      const listingData = {
        ...
        engine: formData.engineSize || '', // ✅ Map engineSize → engine
        capacity: formData.engineSize || '', // ✅ Map dung tích xe → capacity
      ...
      // 🔍 DEBUG: Check buyNowPrice
      console.log('🔍 DEBUG - buyNowPrice GỬI LÊN BACKEND:');
      console.log('   buyNowPrice:', listingData.buyNowPrice, typeof listingData.buyNowPrice);
      console.log('   formData.price:', formData.price, typeof formData.price);
      
      console.log('=== DEBUG MOTO SUBMIT: TRẠNG THÁI ẢNH ===');
      ...
      // BƯỚC 1: Lưu tin dạng Draft trước (theo logic MỚI)
      let response;
      let listingId;
  ...
  return (
    <>
      ...
      >
        <form id="moto-post-form" onSubmit={handleSubmit} noValidate className="space-y-4">
            ...
            <div className="space-y-4">
              ...
              <div className="grid md:grid-cols-2 gap-3">
                ...
                isDarkMode ? 'text-gray-400 hover:text-gray-200' : 'text-gray-400 hover:text-gray-600'
                    ...
                    <div className={`absolute z-10 w-full mt-1 border-2 rounded-lg shadow-lg max-h-60 overflow-y-auto transition-colors duration-500 ${
                      ...
                      {filteredVehicleTypes.length > 0 ? (
                        ...
                        index % 2 === 0 
                          ? (isDarkMode ? 'bg-gray-600' : 'bg-gray-50')
                          : (isDarkMode ? 'bg-gray-700' : 'bg-white')
                      } ${formData.vehicleType === type ? 'bg-[#00c9a7]/10 font-semibold' : ''}`}
                          >
                            {type}
                          </button>
                        ))
                  ...
                  {validationErrors.vehicleType && (
                    <p className="mt-1 text-xs text-red-600">⚠️ {validationErrors.vehicleType}</p>
                  )}
                </div>

                <div className="relative" ref={engineSizeDropdownRef}>
                  <label className={`block text-sm font-semibold transition-colors duration-500 mb-2 ${
                  isDarkMode ? 'text-gray-300' : 'text-gray-700'
                }`}>
                    Dung tích xe <span className="text-red-500">*</span>
                  </label>
                  <div className="relative">
                    <input
                      type="text"
                      value={showEngineSizeDropdown ? engineSizeSearchTerm : formData.engineSize}
                      onChange={(e) => setEngineSizeSearchTerm(e.target.value)}
                      onFocus={() => setShowEngineSizeDropdown(true)}
                      placeholder={formData.engineSize || "Dung tích xe *"}
                      className={`w-full px-3 py-2 pr-8 border-2 rounded-lg focus:outline-none transition-colors text-sm ${
                        isDarkMode ? 'bg-gray-700 text-white border-gray-600' : 'bg-white text-gray-900 border-gray-300'
                      } ${
                    ...
                    <button
                      type="button"
                      onClick={() => setShowEngineSizeDropdown(!showEngineSizeDropdown)}
                      className={`absolute right-3 top-1/2 -translate-y-1/2 transition-colors ${
                isDarkMode ? 'text-gray-400 hover:text-gray-200' : 'text-gray-400 hover:text-gray-600'
              }`}
                    >
                      {showEngineSizeDropdown ? <FiChevronUp /> : <FiChevronDown />}
                    </button>
                  </div>
                  
                  {showEngineSizeDropdown && (
                    <div className={`absolute z-10 w-full mt-1 border-2 rounded-lg shadow-lg max-h-60 overflow-y-auto transition-colors duration-500 ${
                      isDarkMode ? 'bg-gray-700 border-gray-600' : 'bg-white border-gray-300'
                    }`}>
                      {filteredEngineSizes.length > 0 ? (
                        filteredEngineSizes.map((size, index) => (
                          <button
                            key={size}
                            type="button"
                            onClick={() => handleEngineSizeSelect(size)}
                            className={`w-full px-3 py-2 text-left text-sm transition-colors ${
                        isDarkMode 
                          ? 'text-white hover:bg-gray-600' 
                          : 'text-gray-900 hover:bg-gray-100'
                        ...
                        index % 2 === 0 
                          ? (isDarkMode ? 'bg-gray-600' : 'bg-gray-50')
                          : (isDarkMode ? 'bg-gray-700' : 'bg-white')
                      } ${formData.engineSize === size ? 'bg-[#00c9a7]/10 font-semibold' : ''}`}
                          >
                            {size}
                          </button>
                        ))
                  ...
                  {validationErrors.engineSize && (
                    <p className="mt-1 text-xs text-red-600">⚠️ {validationErrors.engineSize}</p>
                  )}
                </div>
              </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="VD: Honda SH 2021 màu đỏ, biển HN"
                  maxLength={50}
                  className={`w-full px-3 py-2 border-2 rounded-lg focus:outline-none transition-colors text-sm ${
                    isDarkMode ? 'bg-gray-700 text-white border-gray-600' : 'bg-white text-gray-900 border-gray-300'
                  } ${
                    ...
                    <div className="space-y-2">
                      ...
                      <div className={`border-t pt-2 mt-2 transition-colors duration-500 ${
                        ...
                        <p className={`text-xs mt-1 transition-colors duration-500 ${
                          ...
                          Xe tay ga Vespa S 150ie 2022 xanh dương mới 95%
                        </p>
                      </div>
                    </div>
                  </motion.div>
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={4}
                  maxLength={1500}
                  placeholder="Mô tả chi tiết về xe: tình trạng, màu sắc, phụ kiện, lịch sử bảo dưỡng..."
